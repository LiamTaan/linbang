...
CREATE TABLE `infra_api_access_log`  (
  ...
  `operate_name` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '' COMMENT '操作名',
  `operate_type` int NOT NULL DEFAULT 0 COMMENT '操作分类',
  ...
  INDEX `idx_application_name`(`application_name` ASC) USING BTREE,
  INDEX `idx_request_url`(`request_url` ASC) USING BTREE,
...
CREATE TABLE IF NOT EXISTS `lb_merchant_entry` (
  ...
  `merchant_name_snapshot` VARCHAR(128) DEFAULT NULL COMMENT '提交时服务商名称快照',
  `contact_name_snapshot` VARCHAR(64) DEFAULT NULL COMMENT '提交时联系人快照',
  `contact_mobile_snapshot` VARCHAR(32) DEFAULT NULL COMMENT '提交时联系电话快照',
  `service_scope_desc_snapshot` VARCHAR(500) DEFAULT NULL COMMENT '提交时服务范围说明快照',
  `applicant_real_name_snapshot` VARCHAR(64) DEFAULT NULL COMMENT '提交时申请人实名快照',
  `category_snapshot_json` LONGTEXT DEFAULT NULL COMMENT '提交时服务类目快照 JSON',
  `qualification_snapshot_json` LONGTEXT DEFAULT NULL COMMENT '提交时资质快照 JSON',
...
CREATE TABLE IF NOT EXISTS `lb_service_category` (
  ...
  `quantity_unit_label` VARCHAR(32) DEFAULT NULL COMMENT '数量口径标签',
  `quantity_split_enabled` BIT(1) NOT NULL DEFAULT b'0' COMMENT '是否允许按数量拆单',
  `split_default_mode` VARCHAR(32) DEFAULT 'DIRECT' COMMENT '默认拆分方式',
  `engineering_category_flag` BIT(1) NOT NULL DEFAULT b'0' COMMENT '是否工程类',
