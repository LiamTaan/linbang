...
public interface PromoterPenaltyRecordMapper extends BaseMapperX<PromoterPenaltyRecordDO> {
    ...
    default List<PromoterPenaltyRecordDO> selectBatchByMinId(Long minId, int limit) {
        int safeLimit = Math.max(1, Math.min(limit, 1000));
        return selectPage(new Page<PromoterPenaltyRecordDO>(1, safeLimit, false),
                new LambdaQueryWrapperX<PromoterPenaltyRecordDO>()
                .gtIfPresent(PromoterPenaltyRecordDO::getId, minId)
                .orderByAsc(PromoterPenaltyRecordDO::getId)).getRecords();
    }
}
