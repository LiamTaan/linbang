...
public class SocialClientApiImpl implements SocialClientApi {
    ...
    public void sendWxaSubscribeMessage(SocialWxaSubscribeMessageSendReqDTO reqDTO) {
        ...
        if (CollUtil.isEmpty(templateList)) {
            log.warn("[sendSubscribeMessage][userType({}) userId({}) 发送订阅消息失败，原因：没有找到订阅模板]",
                    reqDTO.getUserType(), reqDTO.getUserId());
            return;
        }
        // 1.2 获得需要使用的模版
        ...
        if (template == null) {
            log.warn("[sendWxaSubscribeMessage][userType({}) userId({}) 发送订阅消息失败，原因：没有找到订阅模板]",
                    reqDTO.getUserType(), reqDTO.getUserId());
            return;
        }
        ...
        if (ObjUtil.isNull(socialUser) || StrUtil.isBlankIfStr(socialUser.getOpenid())) {
            log.warn("[sendWxaSubscribeMessage][userType({}) userId({}) 发送订阅消息失败，原因：会员 openid 缺失]",
                    reqDTO.getUserType(), reqDTO.getUserId());
            return;
        }
