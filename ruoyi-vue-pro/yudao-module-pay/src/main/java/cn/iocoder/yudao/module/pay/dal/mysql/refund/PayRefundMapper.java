...
public interface PayRefundMapper extends BaseMapperX<PayRefundDO> {
    ...
    default List<PayRefundDO> selectListByStatus(Integer status) {
        return selectList(new LambdaQueryWrapperX<PayRefundDO>()
                .eq(PayRefundDO::getStatus, status)
                .and(wrapper -> wrapper
                        .eq(PayRefundDO::getNeedAudit, false)
                        .or()
                        .isNull(PayRefundDO::getNeedAudit)
                        .or()
                        .eq(PayRefundDO::getAuditStatus, "APPROVED")));
    }

    default int updateByIdAndAuditStatus(Long id, String auditStatus, PayRefundDO update) {
        return update(update, new LambdaQueryWrapper<PayRefundDO>()
                .eq(PayRefundDO::getId, id)
                .eq(PayRefundDO::getAuditStatus, auditStatus));
    }
}
