...
import cn.iocoder.yudao.module.pay.controller.admin.refund.vo.PayRefundAuditReqVO;
...
import cn.iocoder.yudao.module.pay.mq.producer.refund.PayRefundProducer;
...
public class PayRefundServiceImpl implements PayRefundService {
    ...
    @Override
    public void auditRefund(PayRefundAuditReqVO reqVO) {
        PayRefundDO refund = refundMapper.selectById(reqVO.getId());
        if (refund == null) {
            throw exception(REFUND_NOT_FOUND);
        }
        if (!Boolean.TRUE.equals(refund.getNeedAudit())) {
            throw exception(REFUND_AUDIT_NOT_REQUIRED);
        }
        if (!"PENDING".equals(refund.getAuditStatus())) {
            throw exception(REFUND_AUDIT_STATUS_IS_NOT_PENDING);
        }
        if (!"APPROVED".equals(reqVO.getAuditStatus()) && !"REJECTED".equals(reqVO.getAuditStatus())) {
        ...
        PayRefundDO updateObj = new PayRefundDO()
                ...
                .setAuditRemark(reqVO.getAuditRemark())
                .setRejectReason("REJECTED".equals(reqVO.getAuditStatus()) ? reqVO.getRejectReason() : null)
                .setAuditBy(SecurityFrameworkUtils.getLoginUserId())
                .setAuditTime(LocalDateTime.now());
        int updateCounts = refundMapper.updateByIdAndAuditStatus(refund.getId(), refund.getAuditStatus(), updateObj);
        if (updateCounts == 0) {
            throw exception(REFUND_AUDIT_STATUS_IS_NOT_PENDING);
        }
        publishRefundAuditResultMessage(refund, updateObj);
        ...
        try {
            ...
            log.error("[auditRefund][退款 id({}) 审核通过后发起原路退款异常]", refund.getId(), e);
        }
    }
    ...
    public Long createRefund(PayRefundCreateReqDTO reqDTO) {
        ...
        refund = PayRefundConvert.INSTANCE.convert(reqDTO)
                ...
                .setNeedAudit(Boolean.TRUE.equals(reqDTO.getNeedAudit()))
                .setAuditStatus(Boolean.TRUE.equals(reqDTO.getNeedAudit()) ? "PENDING" : "APPROVED")
                .setAuditTime(Boolean.TRUE.equals(reqDTO.getNeedAudit()) ? null : LocalDateTime.now())
        ...
        try {
            submitRefund(refund, order, channel, client);
        } catch (Throwable e) {
            // 注意：这里仅打印异常，不进行抛出。
    ...
    private void submitRefund(PayRefundDO refund, PayOrderDO order, PayChannelDO channel, PayClient<?> client) {
    ...
    private void publishRefundAuditResultMessage(PayRefundDO refund, PayRefundDO updateObj) {
        Long receiverUserId = refund.getUserId();
        if (receiverUserId == null) {
            PayOrderDO order = orderService.getOrder(refund.getOrderId());
            receiverUserId = order != null ? order.getUserId() : null;
        }
        if (receiverUserId == null) {
            return;
        }
        PayRefundAuditResultMessage message = new PayRefundAuditResultMessage();
        message.setRefundId(refund.getId());
        message.setOrderId(refund.getOrderId());
        message.setUserId(receiverUserId);
        message.setAuditStatus(updateObj.getAuditStatus());
        message.setAuditRemark(updateObj.getAuditRemark());
        message.setRejectReason(updateObj.getRejectReason());
        message.setAuditBy(updateObj.getAuditBy());
        message.setAuditTime(updateObj.getAuditTime());
        payRefundProducer.sendRefundAuditResultMessage(message);
    }
