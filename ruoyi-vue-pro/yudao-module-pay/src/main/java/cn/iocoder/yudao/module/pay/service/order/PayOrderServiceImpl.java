...
public class PayOrderServiceImpl implements PayOrderService {
    ...
    @Override
    public PayOrderDetailsRespVO getOrderDetail(Long id) {
        PayOrderDO order = orderMapper.selectById(id);
        if (order == null) {
            return null;
        }
        PayAppDO app = appService.getApp(order.getAppId());
        PayOrderExtensionDO orderExtension = order.getExtensionId() == null ? null : orderExtensionMapper.selectById(order.getExtensionId());
        List<PayOrderExtensionDO> extensions = orderExtensionMapper.selectListByOrderId(order.getId());
        List<PayRefundDO> refunds = payRefundMapper.selectList(new cn.iocoder.yudao.framework.mybatis.core.query.LambdaQueryWrapperX<PayRefundDO>()
                .eq(PayRefundDO::getOrderId, order.getId())
                .orderByDesc(PayRefundDO::getCreateTime, PayRefundDO::getId));

        PayOrderDetailsRespVO respVO = PayOrderConvert.INSTANCE.convert(order, orderExtension, app);
        respVO.setRefunds(PayOrderDetailAssembler.buildRefunds(refunds));
        respVO.setExtensionHistory(PayOrderDetailAssembler.buildExtensionHistory(extensions));
        return respVO;
    }
