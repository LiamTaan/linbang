<template>
  <Dialog v-model="dialogVisible" title="订单详情" width="700px" :loading="detailLoading">
    ...
    <el-divider v-if="detailData.refunds?.length" />
    <el-descriptions
    ...
    >
      <el-descriptions-item label="退款记录">
        ...
        >
          ...
          <div>退款金额：￥{{ (((refund.refundPrice ?? 0) as number) / 100.0).toFixed(2) }}</div>
          <div>审核状态：{{ refund.auditStatus || '-' }}</div>
          <div>退款原因：{{ refund.reason || '-' }}</div>
          <div>渠道退款单号：{{ refund.channelRefundNo || '-' }}</div>
          <div>退款成功时间：{{ formatDate(refund.successTime) }}</div>
          <div>创建时间：{{ formatDate(refund.createTime) }}</div>
        </div>
      </el-descriptions-item>
    </el-descriptions>
    <el-divider v-if="detailData.extensionHistory?.length" />
    <el-descriptions
      v-if="detailData.extensionHistory?.length"
      :column="1"
      label-class-name="desc-label"
      direction="vertical"
      border
    >
      <el-descriptions-item label="支付扩展历史">
        <div
        ...
        >
          ...
          <div>状态：{{ extension.status ?? '-' }}</div>
          <div>用户 IP：{{ extension.userIp || '-' }}</div>
          <div>渠道错误码：{{ extension.channelErrorCode || '-' }}</div>
          <div>渠道错误信息：{{ extension.channelErrorMsg || '-' }}</div>
          <div>创建时间：{{ formatDate(extension.createTime) }}</div>
          <div>更新时间：{{ formatDate(extension.updateTime) }}</div>
          <div style="margin-top: 8px">回调内容：</div>
          <el-text style="overflow-wrap: anywhere; white-space: pre-wrap">
            {{ extension.channelNotifyData || '-' }}
          </el-text>
        </div>
      </el-descriptions-item>
    </el-descriptions>
...
const open = async (id: number) => {
  ...
  try {
    ...
    if (!detailData.value.refunds) {
      detailData.value.refunds = []
    }
    if (!detailData.value.extensionHistory) {
      detailData.value.extensionHistory = []
    }
