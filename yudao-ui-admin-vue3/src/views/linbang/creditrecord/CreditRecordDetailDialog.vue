<template>
  <Dialog v-model="dialogVisible" title="信用记录详情" width="960px" :loading="detailLoading">
    <el-descriptions :column="2" label-class-name="desc-label">
      ...
      <el-descriptions-item label="变动前分值">{{
        detailData.beforeScore ?? '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="变动后分值">{{
        detailData.afterScore ?? '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="当前信用分">{{
        detailData.currentScore ?? '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="当前信用等级">{{
        detailData.creditLevel || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="业务类型">{{
        formatBizType(detailData.bizType)
      }}</el-descriptions-item>
      <el-descriptions-item label="业务对象">{{
        detailData.bizDisplay || '-'
      }}</el-descriptions-item>
    ...
    <el-descriptions :column="2" border>
      <el-descriptions-item label="规则编码">{{
        detailData.rule?.ruleCode || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="规则名称">{{
        detailData.rule?.ruleName || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="规则分值">{{
        detailData.rule?.scoreChange ?? '-'
      }}</el-descriptions-item>
