  ...
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="接单状态" prop="acceptStatus">
        <el-select
          v-model="queryParams.acceptStatus"
          placeholder="请选择接单状态"
          clearable
          class="!w-220px"
        >
  ...
  <ContentWrap>
    ...
    >
      ...
      <el-table-column label="好评率" align="center" width="110">
        <template #default="{ row }">
          {{
            row.positiveRate !== undefined && row.positiveRate !== null
              ? `${row.positiveRate}%`
              : '-'
          }}
        </template>
  ...
  <Dialog v-model="detailVisible" title="服务商详情" width="920px">
    <el-descriptions v-loading="detailLoading" :column="2" border>
      ...
      <el-descriptions-item label="接单状态">{{
        formatAcceptStatus(detailData?.acceptStatus)
      }}</el-descriptions-item>
      <el-descriptions-item label="自动派单">{{
        formatBooleanYesNo(detailData?.dispatchEnabled)
      }}</el-descriptions-item>
      ...
      <el-descriptions-item label="好评率">
        {{
          detailData?.positiveRate !== undefined && detailData?.positiveRate !== null
            ? `${detailData.positiveRate}%`
            : '-'
        }}
      </el-descriptions-item>
      ...
      <el-descriptions-item label="最近入驻区域">{{
    ...
    <el-row :gutter="12" class="mb-16px">
      <el-col :span="8">
        <el-card shadow="never">
          ...
          <div class="mt-8px text-16px font-600">
            <dict-tag
              v-if="detailData?.entryStatus"
              :type="DICT_TYPE.LB_MERCHANT_ENTRY_STATUS"
              :value="detailData.entryStatus"
            />
            <span v-else>-</span>
          </div>
      ...
      <el-col :span="8">
        <el-card shadow="never">
          ...
          <div class="mt-6px text-[var(--el-text-color-secondary)]"
            >接单状态：{{ formatAcceptStatus(detailData?.acceptStatus) }}</div
          >
          <div class="mt-6px text-[var(--el-text-color-secondary)]"
            >自动派单：{{ formatBooleanYesNo(detailData?.dispatchEnabled) }}</div
          >
        </el-card>
      </el-col>
    </el-row>
