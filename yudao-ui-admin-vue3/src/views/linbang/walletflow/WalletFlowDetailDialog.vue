<template>
  <Dialog v-model="dialogVisible" title="钱包流水详情" width="860px" :loading="detailLoading">
    <el-descriptions :column="2" label-class-name="desc-label">
      ...
      <el-descriptions-item label="钱包账户">{{
    ...
    <el-descriptions :column="2" border>
      ...
      <el-descriptions-item label="总资产">{{
        detailData.walletAccount?.totalAmount ?? '-'
      }}</el-descriptions-item>
    ...
    <el-descriptions :column="2" border>
      <el-descriptions-item label="关联订单号">{{
        detailData.order?.orderNo || detailData.relatedOrderNo || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="订单标题">{{
        detailData.order?.title || '-'
      }}</el-descriptions-item>
      ...
      <el-descriptions-item label="支付单">
        {{
          detailData.payOrder?.merchantOrderId ||
          detailData.payOrder?.no ||
          (detailData.order?.payOrderId ? '支付订单信息缺失' : '-')
        }}
      </el-descriptions-item>
    </el-descriptions>
    <el-divider content-position="left">关联单元</el-divider>
    <el-descriptions :column="2" border>
      <el-descriptions-item label="关联单元号">{{
        detailData.unit?.unitNo || detailData.relatedUnitNo || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="单元标题">{{
        detailData.unit?.unitTitle || '-'
      }}</el-descriptions-item>
    ...
    <el-descriptions :column="2" border>
      <el-descriptions-item label="支付订单">
        {{
          detailData.payOrder?.merchantOrderId ||
          detailData.payOrder?.no ||
          (detailData.relatedPayOrderId ? '支付订单信息缺失' : '-')
        }}
      </el-descriptions-item>
      <el-descriptions-item label="支付流水号">{{
        detailData.payOrder?.no || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="支付主题">{{
        detailData.payOrder?.subject || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="支付金额">{{
        detailData.payOrder?.price ?? '-'
      }}</el-descriptions-item>
      ...
      <el-descriptions-item label="支付渠道">{{
        detailData.payOrder?.channelCode || '-'
      }}</el-descriptions-item>
      ...
      <el-descriptions-item label="退款订单">
        {{
          detailData.refund?.merchantRefundId ||
          (detailData.relatedRefundId ? '退款订单信息缺失' : '-')
        }}
      </el-descriptions-item>
      <el-descriptions-item label="退款金额">{{
        detailData.refund?.refundPrice ?? '-'
      }}</el-descriptions-item>
      ...
      <el-descriptions-item label="审核备注">{{
        detailData.refund?.auditRemark || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="驳回原因">{{
        detailData.refund?.rejectReason || '-'
      }}</el-descriptions-item>
    </el-descriptions>
  </Dialog>
</template>
