  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      ...
      <el-form-item label="接单结果" prop="acceptResult">
        <el-select v-model="formData.acceptResult" placeholder="请选择接单结果" clearable>
          <el-option
...
/** 抢单记录 表单 */
defineOptions({ name: 'OrderAcceptRecordForm' })
...
type OrderAcceptRecordFormData = {
  id?: number
  orderId?: number
  unitId?: number
  merchantId?: number
...
const createEmptyFormData = (): OrderAcceptRecordFormData => ({
  id: undefined,
  orderId: undefined,
  unitId: undefined,
...
const formData = ref<OrderAcceptRecordFormData>(createEmptyFormData())
const formRules = reactive({
  orderId: [{ required: true, message: '关联订单不能为空', trigger: 'change' }],
  merchantId: [{ required: true, message: '服务商不能为空', trigger: 'change' }],
  acceptTime: [{ required: true, message: '接单时间不能为空', trigger: 'blur' }],
  acceptResult: [{ required: true, message: '接单结果不能为空', trigger: 'blur' }]
})
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      const detail = await OrderAcceptRecordApi.getOrderAcceptRecord(id)
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as OrderAcceptRecord
    if (formType.value === 'create') {
      await OrderAcceptRecordApi.createOrderAcceptRecord(data)
      message.success(t('common.createSuccess'))
    } else {
      await OrderAcceptRecordApi.updateOrderAcceptRecord(data)
...
const buildFormData = (data: OrderAcceptRecordDetail): OrderAcceptRecordFormData => ({
  id: data.id,
  orderId: data.orderId,
  unitId: data.unitId,
  merchantId: data.merchantId,
  acceptTime: data.acceptTime ? new Date(data.acceptTime as string).getTime() : undefined,
