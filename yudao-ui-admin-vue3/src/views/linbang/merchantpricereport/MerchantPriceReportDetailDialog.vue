<template>
  <Dialog v-model="dialogVisible" title="价格申报详情" width="920px" :loading="detailLoading">
    ...
    <el-descriptions :column="2" border>
      <el-descriptions-item label="入驻单号">{{
        detailData.merchantEntry?.entryNo || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="申请用户">
        {{ formatMerchantEntryUser() }}
      </el-descriptions-item>
      <el-descriptions-item label="入驻区域">{{
        detailData.merchantEntry?.regionCode || '-'
      }}</el-descriptions-item>
    ...
    <el-descriptions :column="4" border>
      <el-descriptions-item label="关联记录数">{{
        detailData.summary?.totalRelatedCount ?? 0
      }}</el-descriptions-item>
...
import {
  ...
  type MerchantPriceReportDetail
...
const formatPartnerDisplay = () => {
  ...
  return (
    [partner?.partnerName, partner?.contactName, partner?.contactMobile]
      .filter(Boolean)
      .join(' / ') || (detailData.value.partnerId ? '合作商信息缺失' : '-')
  )
}
