  ...
  <Dialog v-model="detailVisible" title="反馈统计详情" width="640px">
    <el-descriptions :column="2" border>
      ...
      <el-descriptions-item label="消息分类">{{
        formatMessageCategory(detailData?.messageCategory)
      }}</el-descriptions-item>
      ...
      <el-descriptions-item label="活动 ID">{{
        detailData?.campaignId || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="任务 ID">{{
        detailData?.pushTaskId || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="计划人数">{{
        detailData?.plannedAudienceCount || 0
      }}</el-descriptions-item>
      <el-descriptions-item label="触达人数">{{
        detailData?.reachedCount || 0
      }}</el-descriptions-item>
      <el-descriptions-item label="点击人数">{{
        detailData?.clickedCount || 0
      }}</el-descriptions-item>
      <el-descriptions-item label="已读人数">{{ detailData?.readCount || 0 }}</el-descriptions-item>
      <el-descriptions-item label="语音播放">{{
        detailData?.voicePlayedCount || 0
      }}</el-descriptions-item>
      <el-descriptions-item label="触达率">{{
        formatPercent(detailData?.reachRate)
      }}</el-descriptions-item>
      <el-descriptions-item label="点击率">{{
        formatPercent(detailData?.clickRate)
      }}</el-descriptions-item>
      <el-descriptions-item label="已读率">{{
        formatPercent(detailData?.readRate)
      }}</el-descriptions-item>
    </el-descriptions>
  </Dialog>
</template>
