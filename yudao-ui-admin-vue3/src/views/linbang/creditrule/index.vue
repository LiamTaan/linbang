<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="分值变动" prop="scoreChange">
        <el-input
          v-model="queryParams.scoreChange"
          placeholder="请输入分值变动"
          clearable
          @keyup.enter="handleQuery"
          class="!w-240px"
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['linbang:review:credit-rule:create']"
        >
        ...
        <el-button
          ...
          v-hasPermi="['linbang:review:credit-rule:export']"
        >
      ...
      <el-table-column label="操作" align="center" min-width="120px">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['linbang:review:credit-rule:query']"
          >
            详情
          </el-button>
          <el-button
            ...
            v-hasPermi="['linbang:review:credit-rule:update']"
          >
            编辑
          </el-button>
          <el-button
  ...
  <!-- 表单弹窗：添加/修改 -->
  <CreditRuleForm ref="formRef" @success="getList" />
  <CreditRuleDetailDialog ref="detailDialogRef" />
</template>
...
import { CreditRuleApi, CreditRule } from '@/api/linbang/creditrule'
import {
...
/** 信用分规则 列表 */
defineOptions({ name: 'CreditRule' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<CreditRule[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await CreditRuleApi.getCreditRulePage(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await CreditRuleApi.exportCreditRule(queryParams)
    download.excel(data, '信用分规则.xls')
  } catch {
  } finally {
