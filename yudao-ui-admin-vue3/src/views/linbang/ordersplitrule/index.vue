    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['linbang:order:split-rule:create']"
        >
    ...
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="计价方式" min-width="180">
        <template #default="{ row }">{{
          (row.applicablePricingModes || []).join(' / ') || '全部'
        }}</template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="160" fixed="right">
        <template #default="{ row }">
          <el-button
            ...
            v-hasPermi="['linbang:order:split-rule:update']"
            >编辑</el-button
          >
          <el-button
            link
            ...
            v-hasPermi="['linbang:order:split-rule:delete']"
            >删除</el-button
          >
        </template>
      </el-table-column>
...
const handleDelete = async (id: number) => {
  try {
    ...
    const verifyToken = await requestDynamicKeyToken('删除订单拆单规则')
    await OrderSplitRuleApi.delete(id, verifyToken)
