<template>
  <Dialog v-model="dialogVisible" title="佣金详情" width="920px" :loading="detailLoading">
    <el-descriptions :column="2" label-class-name="desc-label">
      <el-descriptions-item label="佣金单号">{{
        detailData.commissionNo || '-'
    ...
    <el-descriptions :column="2" border>
      <el-descriptions-item label="推广员编号">{{
        detailData.promoter?.userNo || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="推广员手机号">{{
        detailData.promoter?.userMobile || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="推广员昵称">{{
        detailData.promoter?.userNickname || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="推广员等级">{{
    ...
    <el-descriptions :column="2" border>
      <el-descriptions-item label="订单号">{{
        detailData.sourceOrder?.orderNo || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="订单标题">{{
        detailData.sourceOrder?.title || '-'
      }}</el-descriptions-item>
      <el-descriptions-item label="订单金额">{{
        detailData.sourceOrder?.orderAmount ?? '-'
      }}</el-descriptions-item>
    ...
    <el-descriptions :column="2" border>
      ...
      <el-descriptions-item label="完成时间">{{
        formatDate(detailData.sourceUnit?.finishTime)
      }}</el-descriptions-item>
    </el-descriptions>
  </Dialog>
</template>
...
const formatPromoterDisplay = () => {
  const summary = [
    detailData.value.promoter?.userNickname,
    detailData.value.promoter?.userMobile,
    detailData.value.promoter?.userNo
  ]
    .filter(Boolean)
    .join(' / ')
